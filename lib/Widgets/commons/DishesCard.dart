...
class DishesCardState extends State<DishesCardWidget> {
  ...
  Future<DocumentSnapshot> getUsers() async {
    ...
    DocumentSnapshot yourResult =
      ...
      DocumentSnapshot cuisto =
        await Firestore.instance.collection("users").document(widget.document["user_id"]).get();
    setState(() {
      yourUser = yourResult;
      user=cuisto;
      _isCharging = false;
  ...
  bool _isMatchingInterests() {
    print("isMatchingInterests");
    ...
    if (user["centres_interet"] != null) {
      user["centres_interet"].forEach((item) {
        print("foreaching cuisto");
        yourUser["centres_interet"].forEach((yourItem) {
          print("foreaching connected user");
          if (item == yourItem) {
            if (matchingItems.contains(item)) {
            } else {
              print("adding:" + item + " to matching list.");
              matchingItems.add(item);
              numberOfMatching += 1;
              print(
                  "current matching occurences:" + numberOfMatching.toString());
            }
          }
        });
      });
    }
    return numberOfMatching >= 3;
  }
